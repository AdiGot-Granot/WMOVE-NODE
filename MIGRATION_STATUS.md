...
| 3b | MPEST | `entrywc` (Local/LD) | form | `MPEST.PRG:444` | Estimate entry form (`entry.wc`, default Local/Long-Distance branch). Read/render + lookups (TREF, MOVESIZE) done & tested. **Create path verified on DEMOPRO** (job #578106 via `cst.NextJobNo` OUTPUT params). Write stays gated by `ESTIMATE_WRITES` (set true in db mode to enable). |
| 3c | MPEST | `entrywc` (International) | form | `MPEST.PRG:444` (`plINTL` branch) | International branch of `entrywc` — reached as `?module=INT` from `selectestret`. Same controller/service/template as Local; `TYPE='I'` is set on the new estimate, the title becomes "International Entry Form" and origin/destination Country fields are rendered. Same write gating as 3b. |
| 3d | MPEST | `entryautowc` | form | `MPEST.PRG:235` | Auto Transport entry form (`entryauto.wc`). Renders the AUTO branch of `selectestret`; creates a `TYPE='T'` estimate (write-gated by `ESTIMATE_WRITES` in db mode) and binds the auto-transport location options (Residence/Business/Dealership/Port/Terminal/Auction). Department picker shown only when `COLEVEL=3` and the tenant has multiple active departments. |
...
### MPEST endpoint detail (4 of 36 done)

Done: `indexwc`, `entrywc` (Local/LD + International + Broker), `entryautowc`, `inventory`. Remaining stubs (each returns its source line):
